...
class Model(nn.Module):
    ...
    def forward(self, inputs, text):
        ...
        # device-aware mask, same shape & device as nfea_magnitudes
        select_idx = torch.ones_like(nfea_magnitudes, device=device)
        select_idx = self.drop_out(select_idx)
        afea_magnitudes_drop = afea_magnitudes * select_idx
        
        idx_abn = torch.topk(afea_magnitudes_drop, k_abn, dim=1)[1]  # (n_size, k_abn)
        idx_abn_feat = idx_abn.unsqueeze(2).expand([-1, -1, abnormal_features.shape[2]])  # (n_size, k_abn, feat_dim)
        
        abnormal_features = abnormal_features.view(n_size, ncrops, t, -1)  # (n_size, ncrops, t, feat_dim)
        abnormal_features = abnormal_features.permute(1, 0, 2, 3)          # (ncrops, n_size, t, feat_dim)
        
        # start with a proper 3D, zero-length tensor on the *same device & dtype*
        total_select_abn_feature = features.new_zeros(
            (0, idx_abn_feat.shape[1], idx_abn_feat.shape[2])
        )  # (0, k_abn, feat_dim)
        
        for abnormal_feature in abnormal_features:  # abnormal_feature: (n_size, t, feat_dim)
            feat_select_abn = torch.gather(abnormal_feature, 1, idx_abn_feat)  # (n_size, k_abn, feat_dim)
            total_select_abn_feature = torch.cat((total_select_abn_feature, feat_select_abn), dim=0)
        
        idx_abn_score = idx_abn.unsqueeze(2).expand([-1, -1, abnormal_scores.shape[2]])  # (n_size, k_abn, 1)
        score_abnormal = torch.mean(torch.gather(abnormal_scores, 1, idx_abn_score), dim=1)  # (n_size, 1)
        
        ####### process normal videos -> select top3 feature magnitude #######
        
        select_idx_normal = torch.ones_like(nfea_magnitudes, device=device)
        select_idx_normal = self.drop_out(select_idx_normal)
        nfea_magnitudes_drop = nfea_magnitudes * select_idx_normal
        
        idx_normal = torch.topk(nfea_magnitudes_drop, k_nor, dim=1)[1]  # (n_size, k_nor)
        idx_normal_feat = idx_normal.unsqueeze(2).expand([-1, -1, normal_features.shape[2]])  # (n_size, k_nor, feat_dim)
        
        normal_features = normal_features.view(n_size, ncrops, t, -1)
        normal_features = normal_features.permute(1, 0, 2, 3)
        
        total_select_nor_feature = features.new_zeros(
            (0, idx_normal_feat.shape[1], idx_normal_feat.shape[2])
        )  # (0, k_nor, feat_dim)
        
        for nor_fea in normal_features:  # (n_size, t, feat_dim)
            feat_select_normal = torch.gather(nor_fea, 1, idx_normal_feat)  # (n_size, k_nor, feat_dim)
            total_select_nor_feature = torch.cat((total_select_nor_feature, feat_select_normal), dim=0)
        
        idx_normal_score = idx_normal.unsqueeze(2).expand([-1, -1, normal_scores.shape[2]])  # (n_size, k_nor, 1)
        score_normal = torch.mean(torch.gather(normal_scores, 1, idx_normal_score), dim=1)   # (n_size, 1)
