...
def test(dataloader, model, args, viz, device):
    with torch.no_grad():
        ...
        for i, (input, text) in enumerate(tqdm(dataloader, desc="Testing", ncols=100)):  # test set has 199 videos
            ...
            scores_nor_bottom, scores_nor_abn_bag, feat_magnitudes = model(input, text)
